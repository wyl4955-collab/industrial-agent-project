...
> NLP/视觉组: `load_finetuned_model()` 接口已预留, 微调模型加载后自动切换, 在技术报告中附上微调前后指标对比即可兑现 15 分。
...
**Q: 可以用中文作为 class_name 吗?**
A: 建议用 ObjectClass 枚举中的英文值, 避免编码问题。NLP 模块负责中文→英文的映射。
